...
// |||||||||||| NEW STUFF
int currentUserBooking[MAX_ITEM_COUNT];
int currentUserBookingCount;
// |||||||||||| NEW STUFF
...
// |||||||||||||||||||||||||||||| NEW STUFF
// you can rename this to something better, i was lazy
void display_currentUserBooking() {
  printf("\n"
         "--TYPE-----ID---ROOMS---$/NIGHT---MAXGST---AMENITIES-------------"
         "\n");
  for (int i = 0; i < currentUserBookingCount; i++) {
    int i2 = currentUserBooking[i];
    printf("| %-8s %-4d %-7d %-9d %-8d %-20s |\n",
           STORAGE_ROOM_TYPES[i2],          //
           i2,                              //
           STORAGE_ROOM_NUMROOMS[i2],       //
           STORAGE_ROOM_PRICESPERNIGHT[i2], //
           STORAGE_ROOM_MAXGUESTS[i2],      //
           STORAGE_ROOM_AMENITIES[i2]       //
    );
  }
  printf("-----------------------------------------------------------------\n");
}
...
void display_roomSelection_numRoomsOpts() {
  ...
  printf("\n"
         "- Please enter how many ROOMS the units you want to book have\n"
         "\n"
         "Option: ");
}
...
// ||||||||||||||||||||| NEW STUFF
void fill_currentUserBooking(char type[], int numRooms) {
  currentUserBookingCount = 0;
  for (int i = 0; i < MAX_ITEM_COUNT; i++) {
    if (streq(type, STORAGE_ROOM_TYPES[i]) &&
        numRooms <= STORAGE_ROOM_NUMROOMS[i]) {
      currentUserBooking[currentUserBookingCount] = i;
      currentUserBookingCount++;
    }
  }
}
void clear_currentUserBooking() {
  currentUserBookingCount = 0;
  for (int i = 0; i < MAX_ITEM_COUNT; i++) {
    currentUserBooking[i] = 0;
  }
}
...
int INT_roomSelection_reserve() {
  ...
  // |||||||||||||||||| NEW STUFF |||||||||||||||||||||||
  // fills up the user's array with the ids of the rooms that meet their
  // criteria
  fill_currentUserBooking(desiredType, currentUserInt);
  // |||||||||||||||||| NEW STUFF |||||||||||||||||||||||
  ...
  if (count < 1) {
    ...
    printf("\n"
           "- Noted! Out of %d, we have %d rooms of type '%s', which have at "
           "least "
           "%d rooms\n"
           "\n"
           "- Please proceed to the Booking summary for payment and "
           "confirmation.\n",
           maxcount, count, desiredType, currentUserInt);
  }
  prompt_wait();
  currentState = STATE_ROOMSELECTION;
  return 0;
...
int INT_roomSelection_summary() { // for choices. don't override the current
                                  // state to ensure that they run
  clear_screen();
